...
if (!(defined('MULTISITE') && MULTISITE)) {
	...
	function aletheme_theme_activation_init() {
		...
		register_setting(
			'aletheme_activation',
            'aletheme_theme_activation',
            'aletheme_theme_activation_validate'
		);
	}
	...
	add_filter('option_page_capability_aletheme_activation',
               'aletheme_activation_page_capability');
	...
	function aletheme_theme_activation_add_page() {
		...
		if (!$aletheme_activation['first_run']) {
			add_theme_page( __('Theme Activation', 'aletheme'),
                            __('Theme Activation', 'aletheme'),
                            'edit_theme_options',
                            'aletheme_theme_activation',
                            'aletheme_theme_activation_render_page');
		} else {
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<?php if (!$config_writable) : ?>
					<div class="error">
						<p>If your <code>.htaccess</code> file were
                            <a href="http://codex.wordpress.org/Changing_File_Permissions" target="_blank">writable</a>,
                            we could do this automatically, but it isn&#8217;t so these are the mod_rewrite rules you should have in your
                            <code>.htaccess</code> file. Please go to
                            <a href="<?php echo admin_url('options-permalink.php') ?>">Permalinks</a>
                            page in your <strong>Settings</strong> tab and adjust your links manually.
                        </p>
					</div>
				<?php endif; ?>
				...
				<table class="form-table">
					...
					<tr valign="top">
                        ...
                        <th scope="row">
                            <?php _e('Create static front page?', 'aletheme'); ?>
                        </th>
					...
					<tr valign="top">
                        ...
                        <th scope="row">
                            <?php _e('Create a blog page?', 'aletheme'); ?>
                        </th>
					...
					<tr valign="top">
                        ...
                        <th scope="row">
                            <?php _e('Change permalink structure?', 'aletheme'); ?>
                        </th>
						...
						<td>
							...
							<fieldset>
                                ...
                                <legend class="screen-reader-text">
                                    <span>
                                        <?php _e('Update permalink structure?', 'aletheme'); ?>
                                    </span>
                                </legend>
								...
								<?php if ($config_writable) : ?>
									...
									<span class="description">
                                        <?php printf(__('Change permalink structure to /&#37;postname&#37;/', 'aletheme')); ?>
                                    </span>
