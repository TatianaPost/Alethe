...
function ale_dropdown_add_blank_item( $items, $args ) {
    if ( isset( $args->walker ) && is_object( $args->walker ) && method_exists( $args->walker, 'is_dropdown' ) ) {
        ...
        $title = isset( $args->dropdown_title ) ?
                            wptexturize( $args->dropdown_title ) :
                                '&mdash; ' .
                  (isset($args->menu->name) ?
                            $args->menu->name :
                                ...
                                ' &mdash;';
        if ( ! empty( $title ) )
            $items = '<option value="" class="blank">' .
                        apply_filters( 'dropdown_blank_item_text', $title, $args ) .
                     '</option>' .
                     $items;
    }
    return $items;
...
function ale_dropdown_remove_empty_items( $items, $args ) {
    if ( isset( $args->walker ) &&
         ...
         method_exists( $args->walker, 'is_dropdown' ) )
...
if (is_admin()) {
    ...
    function ale_inject_archives_menu_meta_box() {
        add_meta_box('ale-nav-advanced',
                     __( 'Advanced Navigation', 'aletheme' ),
                     'ale_wp_nav_menu_archives_meta_box',
                     'nav-menus',
                     'side',
                     'default');
    }
