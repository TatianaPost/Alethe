...
function ale_add_admin_menu() {
    add_theme_page( __('Theme Options', 'aletheme'),
	                __('Theme Options', 'aletheme'),
	                'edit_theme_options',
	                'aletheme',
	                'optionsframework_page');
    add_theme_page(__('Sliders', 'aletheme'),
	               __('Sliders', 'aletheme'),
	               'edit_theme_options',
	               'aletheme_sliders',
	               'aletheme_sliders_page');
}
...
function ale_admin_notice_tagline() {
	if ((get_option('blogdescription') === 'Just another WordPress site')) {
		...
		echo '<div class="error">';
			echo '<p>
						<strong>', sprintf(__('Please update your <a href="%s">site tagline</a>', 'aletheme'),
		...
		foreach (aletheme_get_post_types() as $type => $config) {
			if (isset($config['columns']) && count($config['columns'])) {
				foreach ($config['columns'] as $column) {
					if (function_exists('ale_admin_posts_' . $column . '_column_head') &&
					    function_exists('ale_admin_posts_' . $column . '_column_content')) {
...
function ale_admin_remove_footer_admin() {
	echo '<span id="footer-thankyou">
			Developed by <a href="'.'http://www.alethemes.com'.'" target="_blank">Alethemes</a>
		 </span>';
}
