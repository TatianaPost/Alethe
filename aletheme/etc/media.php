...
function ale_attachment_fields_to_edit($form_fields, $post) {
	...
	$form_fields["hide_from_gallery"] = array(
		...
		'html'	=> "<label>
						<input type='checkbox' 
							   value='1' {$checked} 
							   name='attachments[{$post->ID}][hide_from_gallery]' 
							   id='attachments[{$post->ID}][hide_from_gallery]' /> 
							   Don't display image in gallery.
					</label>",
		"value" => $val,
	);
		...
		foreach (aletheme_media_custom_fields() as $field) {
			...
			switch ($field['type']) {
				...
				case 'checkbox':
					...
					$data['html'] = '<label>
										<input type="checkbox" 
											   value="1" ' . ($data['value'] ? 'checked="checked"' : '') . ' 
											   name="attachments[' . $post->ID . '][' . $field['name'] . ']" 
											   id="attachments[' . $post->ID . '][' . $field['name'] . ']" />';
					break;
...
function ale_attachment_fields_to_save($post, $attachment) {
	update_post_meta($post['ID'], '_ale_hide_from_gallery', isset($attachment['hide_from_gallery']) ?
		(int) $attachment['hide_from_gallery'] :
			0);
...
function ale_gallery_metabox_add() {
	...
	foreach ($post_types as $post_type) {
		...
		if( apply_filters( 'ale_gallery_metabox_limit', true, $post_id ) ) {
			add_meta_box( 'ale_gallery_metabox',
					       __( 'Gallery Images', 'gallery-metabox' ),
						  'ale_gallery_metabox',
					      $post_type,
					      $context,
						  $priority );
...
function ale_gallery_metabox_html($post_id) {
    ...
    $intro = '<p>
				<a href="media-upload.php?post_id=' .
                   $post_id .
                   '&amp;type=image&amp;TB_iframe=1&amp;width=640&amp;height=715" id="add_image" class="thickbox" title="' .
                    __( 'Add Image', 'aletheme' ) . '">' .
                    __( 'Upload Images', 'aletheme' ) .
                '</a> | 
                <a href="media-upload.php?post_id=' .
                   $post_id .
                   '&amp;type=image&amp;tab=gallery&amp;TB_iframe=1&amp;width=640&amp;height=715" id="manage_gallery" class="thickbox" title="' .
                   __( 'Manage Gallery', 'aletheme' ) . '">' .
                   __( 'Manage Gallery', 'aletheme' ) .
                '</a>
              </p>
	<p>
		<b style="color:#990000;">
			To remove the image from gallery, delete it permanently or check the options "Don\'t display image in gallery." in image properties
			</b>
	</p>';
    ...
    if (empty($loop)) {
        return '<p>
					Gallery is empty. You must upload new images. You are not able to use old images uploaded from Media Library. 
						<b style="color:#990000;">
							After uploading close the modal window, don\'t insert images into post.
						</b>
				</p>
				<a href="media-upload.php?post_id=' .
                   $post_id .
	...
	foreach ($loop as $image) {
		...
		$return   .= apply_filters('ale_gallery_metabox_output',
								   '<img src="' . $thumbnail[0] . '" alt="' . $image->post_title . '" title="' . $image->post_content . '" /> ',
								   $thumbnail[0],
								   $image );
	}
