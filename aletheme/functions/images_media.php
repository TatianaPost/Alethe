...
function ale_remove_first_image($content = null) {
    ...
    $content = trim(preg_replace('~(<a[^>]+>)?\s*(<img[^>]+>)\s*(</a>)?~sim',
                                 '',
                                  $content,
                                 1)
                    );
...
{
	return preg_replace('~<a[^>]*>(<img[^>]*>)<\/a>~iu',
                        '$1',
                         $content);
}
 ...
 * @return string 
 */
function ale_display_first_attached_image($id = null,
                                          $size = 'large',
                                          $show_hidden = true)
{
...
function ale_create_youtube_player($media_source = '', $width = 640, $height = 360, $allow_autoplay = 1) {
	if (preg_match('%(?:youtube\.com/(?:user/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i',
                    $media_source,
                    $matches)) {
		...
		$output = __('Sorry that seems to be an invalid <strong>YouTube</strong> URL. Please check it again.',
                     'aletheme');
	}
 ...
 * @return string
 */
function ale_create_vimeo_player($media_source = '',
                                 $width = 640,
                                 $height = 360,
                                 $allow_autoplay = 1) {
	...
	if (preg_match('~^http://(?:www\.)?vimeo\.com/(?:clip:)?(\d+)~', $media_source, $matches)) {
		...
		$video_color = ( 1 == ale_get_option('enable_styles') && $color ) ?
                         ltrim($color, '#') :
                         '252A31';
		...
		$output = __('Sorry that seems to be an invalid <strong>Vimeo</strong> URL. 
                      Please check it again. 
                      Make sure there is a string of numbers at the end (e.g. http://vimeo.com/1234567).',
                    'aletheme');
	}
	return $output;
}
 ...
 * @return string
 */
function ale_create_wp_embed_player( $media_source = '',
                                     $width = 640,
                                     $height = 360,
                                     $allow_autoplay = 1 ) {

	$wp_embed = new WP_Embed();
	$output = $wp_embed->run_shortcode( '[embed width=' . $width . ' height=' . $height . ']' .
                                          $media_source . '[/embed]' );
	return $output;
}
...
function ale_lazyload_images($content) {
	...
	foreach ($_matches[0] as $match) {
		...
		if (!substr_count($match, 'no-lazyload')) {
			$update = preg_replace('~src="([^"]+)"~sim','src="' .
                                    $pre . '" data-original="$1"',
                                    $match);
...
function ale_bulk_lazyload_images($content) {
	$content = preg_replace('~src="([^"]+)"~sim','src="' .
                            $pre . '" data-original="$1"',
                            $content);
	return $content;
}