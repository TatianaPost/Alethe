...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET[sanitize_key( 'plugin' )] ) &&
                ...
                $url   = add_query_arg( array( 'action' => 'install-plugin',
                                                'plugin' => $plugin['slug'] ),
                                        'update.php' );
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        echo '<p>
                                <a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) .
                                    '" title="' . esc_attr( $this->strings['return'] ) .
                                    '" target="_parent">' . __( 'Return to Required Plugins Installer', $this->domain ) . '
                                </a>
                              </p>';
                ...
                foreach ( $this->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        ...
                        echo '<p>
                                <a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) .
                                    '" title="' . esc_attr( $this->strings['return'] ) .
                                    '" target="_parent">' . __( $this->strings['return'], $this->domain ) . '
                                </a>
                              </p>';
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'],
                            '<a href="' . admin_url() .
                             '" title="' . __( 'Return to the Dashboard', $this->domain ) . '">' .
                             __( 'Return to the Dashboard', $this->domain ) . '</a>' ) . '
                          </p>';
            ...
            elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) &&
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
                        ...
                        $msg = sprintf( __( 'The following plugin was activated successfully: %s.', $this->domain ),
        ...
        public function notices() {
                ...
                if ( ! empty( $message ) ) {
                    ...
                    foreach ( $message as $type => $plugin_groups ) {
                        ...
                        foreach ( $plugin_groups as $plugin_group_single_name ) {
                            ...
                            if ( $external_url && preg_match( '|^http(s)?://|', $external_url ) ) {
                                ...
                                $linked_plugin_groups[] = '<a href="' . esc_url( $external_url ) .
                                                          '" title="' . $plugin_group_single_name .
                                                          '" target="_blank">' . $plugin_group_single_name .
                                                          '</a>';
                            }
                            elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
                                ...
                                $linked_plugin_groups[] = '<a href="' . esc_url( $url ) .
                                                          '" class="thickbox" title="' . $plugin_group_single_name . '">' .
                                                          $plugin_group_single_name .
                                                          '</a>';
                            }
                            else {
                        ...
                        $imploded    = empty( $plugin_groups ) ?
                            '<em>' . $last_plugin . '</em>' : '<em>' . ( implode( ', ', $plugin_groups ) .
                                                                            '</em> and <em>' . $last_plugin . '</em>' );

                        /* All messages now stored */
                        $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, $this->domain ),
                                             $imploded, $count ) .
                                     '</p>';
                    }

                    /** Setup variables to determine if action links are needed */
                    $show_install_link  = $install_link ?
                        '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">'
                            . translate_nooped_plural( $this->strings['install_link'], $install_link_count, $this->domain ) .
                        '</a>' :
                            '';

                    $show_activate_link = $activate_link ?
                        '<a href="' . admin_url( 'plugins.php' ) . '">' .
                            translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, $this->domain ) .
                        '</a>'  :
                            '';
                    ...
                    $action_links = apply_filters(
                        ...
                        array(
                            ...
                            'dismiss'  => '<a class="dismiss-notice" 
                                              href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" 
                                              target="_parent">' .
                                                __( 'Dismiss this notice', $this->domain ) .
                                          '</a>',
                        )
                    );
            ...
            if(isset($_GET['page'])){
                if ($_GET['page'] == 'aletheme') {
                    $themeoptions_page = 'aletheme';
                } elseif($_GET['page'] == 'aletheme_theme_activation') {
                    $themeoptions_page = 'aletheme_theme_activation';
                }
            } else {
                $themeoptions_page = '';
            }
            ...
            if (( $pagenow == 'themes.php' ) && ($themeoptions_page == 'aletheme')) {
                //Don't show errors
            } else if(( $pagenow == 'themes.php' ) && ($themeoptions_page == 'aletheme_theme_activation')){
                //Don't show errors
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( $external_url && preg_match( '|^http(s)?://|', $external_url ) ) {
                    $table_data[$i]['plugin'] = '<strong>
                                                    <a href="' . esc_url( $external_url ) .
                                                       '" title="' . $plugin['name'] .
                                                       '" target="_blank">' . $plugin['name'] . '
                                                    </a>
                                                 </strong>';
                }
                elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
                ...
                $table_data[$i]['type'] = $plugin['required'] ?
                    __( 'Required', TGM_Plugin_Activation::$instance->domain ) :
                    __( 'Recommended', TGM_Plugin_Activation::$instance->domain );
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. 
                            <a href="%1$s" 
                                title="Return to the Dashboard">Return to the Dashboard
                            </a>',
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                foreach ( $plugin_installs as $plugin ) {
                    $api[$c] = plugins_api( 'plugin_information', array( 'slug' => $plugin,
                                                                         'fields' => array( 'sections' => false ) ) ) ?
                        plugins_api( 'plugin_information', array( 'slug' => $plugin,
                                                                  'fields' => array( 'sections' => false ) ) ) :
                ...
                foreach ( $api as $object ) {
                    $sources[$d] = isset( $object->download_link ) && 'repo' == $plugin_paths[$d] ?
                ...
                $url   = add_query_arg( array( 'page' => TGM_Plugin_Activation::$instance->menu ),
                                                admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                $plugins = isset( $_POST[sanitize_key( 'plugin' )] ) ?
                    (array) $_POST[sanitize_key( 'plugin' )] :
                    array();
                ...
                $imploded    = empty( $plugin_names ) ?
                    '<strong>' . $last_plugin . '</strong>' :
                    '<strong>' . ( implode( ', ', $plugin_names ) . '</strong> and <strong>' . $last_plugin . '</strong>.' );
                ...
                if ( is_wp_error( $activate ) )
                    echo '<div id="message" class="error">
                            <p>' . $activate->get_error_message() . '</p>
                         </div>';
                else
                    printf( '<div id="message" class="updated">
                                    ...
                                    _n( 'The following plugin was activated successfully:',
                                        ...
                                        $count, TGM_Plugin_Activation::$instance->domain ),
                    $imploded );
...
if ( ! class_exists( 'WP_Upgrader' ) &&
    ( isset( $_GET[sanitize_key( 'page' )] ) &&
        TGM_Plugin_Activation::$instance->menu = $_GET[sanitize_key( 'page' )] ) ) {
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. 
                                                                                 This process may take a while on some hosts, so please be patient.',
                                                                               TGM_Plugin_Activation::$instance->domain );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.',
                                                                                    TGM_Plugin_Activation::$instance->domain ) .
                                                                                    ' <a onclick="%2$s" href="#" class="hide-if-no-js">
                                                                                          <span>' .
                                                                                                __( 'Show Details', TGM_Plugin_Activation::$instance->domain ) .
                                                                                          '</span>
                                                                                          <span class="hidden">' .
                                                                                                __( 'Hide Details', TGM_Plugin_Activation::$instance->domain ) .
                                                                                          '</span>.
                                                                                    </a>';
                ...
                /** Default installation strings */
                    ...
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. 
                                                                                 This process may take a while on some hosts, so please be patient.',
                                                                                TGM_Plugin_Activation::$instance->domain );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing 
             ...
             * @since 2.2.0
             */
            public function before($args = array()) {
             ...
             * @since 2.2.0
             */
            public function after($args = array()) {
                ...
                if ( ! empty( $this->result ) && ! is_wp_error( $this->result ) ) {
                    echo '<div class="updated">
                            <p>' . sprintf( $this->upgrader->strings['skin_update_successful'],
            ...
            public function bulk_footer() {
                ...
                foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        echo '<p>
                                <a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu,
                                               admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) ) .
                                '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) .
                                '" target="_parent">'
                ...
                if ( empty( $complete ) ) {
                    echo '<p>'
                                .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'],
                                   '<a href="' . admin_url() . '" 
                                            title="' . __( 'Return to the Dashboard', TGM_Plugin_Activation::$instance->domain ) . '">' .
                                               __( 'Return to the Dashboard', TGM_Plugin_Activation::$instance->domain ) .
                                         '</a>' ) .
                        '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
