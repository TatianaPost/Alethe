...
if ( ! class_exists( 'Aletheme_export_import_options' ) ) {
    ...
    class Aletheme_export_import_options
    {
        ...
        function import_export()
        {
            ...
            if (isset($_POST['upload']) && check_admin_referer('alethemesframework_restoreOptions', 'alethemesframework_restoreOptions')) {
                if ($_FILES["file"]["error"] > 0) {
                    ...
                    if(!is_object($widget_file)){
                        ...
                        wp_die(
                            __( 'Import data could not be read. Please try a different file. 
                                 Make sure you upload a <strong>Theme Options</strong> file and not Widgets file.',
                                 'aletheme' ),
                            '',
                            array( 'back_link' => true )
                        );
            ...
            if (isset($_POST['upload_widget']) && check_admin_referer('alethemeswidget_restoreOptions',
                                                                      'alethemeswidget_restoreOptions')) {
        ...
        function ale_import_data( $data ) {
            ...
            if ( empty( $data ) || ! is_object( $data ) ) {
                wp_die(
                    __( 'Import data could not be read. Please try a different file. 
                         Make sure you upload a <strong>Widgets</strong> file and not Theme Options file.',
                        'aletheme' ),
                    '',
                    array( 'back_link' => true )
                );
                ...
                foreach ( $widgets as $widget_instance_id => $widget ) {
                    ...
                    if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
                        ...
                        $sidebar_widgets = isset( $sidebars_widgets[$use_sidebar_id] ) ?
                            ...
                            array(); // check Inactive if that's where will go
                    ...
                    if ( ! $fail ) {
                        ...
                        $single_widget_instances = ! empty( $single_widget_instances ) ?
                            ...
                            array( '_multiwidget' => 1 ); // start fresh if have to
                    ...
                    $results[$sidebar_id]['widgets'][$widget_instance_id]['name'] = isset( $available_widgets[$id_base]['name'] ) ?
                        $available_widgets[$id_base]['name'] :
                        $id_base; // widget name or ID if name not available (not supported by site)
                    $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ?
                        $widget['title'] :
                        __( 'No Title', 'aletheme' ); // show "No Title" if widget instance is untitled
            ...
            <div class="wrap">
                ...
                <div class="cf">
                    ...
                    <div class="item-importer">
                        ...
                        <form action=""
                            ...
                            <div id="backup-options">
                                ...
                                <p>
                                    <a href="?page=backup-options&action=download" class="button button-primary">
                                        <?php _e( 'Export Theme Options', 'aletheme' ); ?>
                                    </a>
                                </p>

                                <h3><?php _e('Import Theme Options', 'aletheme'); ?>:</h3>
                                <p>
                                    <?php _e( 'Please select a 
                                               <b>.dat</b> 
                                               file (<span class="red">with Theme Options</span>) generated by this theme.',
                                                'aletheme' );
                                    ?>
                                </p>
                                ...
                                <p>
                                    ...
                                    <input type="submit"
                                           ...
                                           value="<?php _e('Import Theme Options', 'aletheme'); ?>"
                                    />
                                </p>
                    ...
                    <div class="item-importer">
                        ...
                        <p>
                            <a href="?page=backup-options&export=1"
                               id="wie-export-button"
                               class="button button-primary"><?php _e( 'Export Widgets', 'aletheme' ); ?>
                            </a>
                        </p>
                        ...
                        <p>
                            <?php _e( 'Please select a <b>.dat</b> 
                                       file (<span class="red">with widgets</span>) generated by this theme.',
                                     'aletheme' );
                            ?>
                        </p>
                        ...
                        <form method="post" enctype="multipart/form-data">
                            <p>
                                ...
                                <input type="submit"
                                       ...
                                       id="upload_widget"
                                       class="button-primary"
                                       value="<?php _e('Import Widgets', 'aletheme'); ?>"
                                />

                            </p>
                            <?php if (function_exists('wp_nonce_field')) {
                                wp_nonce_field('alethemeswidget_restoreOptions',
                                               'alethemeswidget_restoreOptions');
                            }

                            if(isset($_GET["success_widget"]) && $_GET["success_widget"]==true) {
                                echo "<p class='saved success'>".
                                         __('Widgets were imported with success.',
                                             'aletheme')."
                                      </p>";
                            }?>
                    ...
                    <div class="item-importer">
                        ...
                        <p>
                            <?php _e('You should install the plugin <strong>WordPress Importer</strong> and import the xml file with demo content. 
                                     This file will import into your theme all demo posts, pages and media files.',
                                     'aletheme'); ?>
                        </p>

                        <p>
                            <?php _e('You can install the plugin manually or go to <strong>Tools > Import > WordPress</strong> and install/import the demo content file.',
                                    'aletheme'); ?>
                        </p>

                        <p>
                            <?php _e('Also make sure you checked the option "Download and import file attachments".','aletheme'); ?>
                        </p>

                        <p>
                            <a href="https://wordpress.org/plugins/wordpress-importer/"
                               target="_blank"
                               class="button-primary">
                                    <?php _e('Download WordPress Importer','aletheme'); ?>
                            </a>
                        </p>
        ...
        {
            global $wpdb;
            return $wpdb->get_results("SELECT option_name, 
                                       option_value FROM {$wpdb->options} 
                                       WHERE option_name = 'alethemesframework'"
            ); // edit 'option_name' to match theme options
        }
    }
}
