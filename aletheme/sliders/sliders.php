...
class Aletheme_Sliders
{
	...
	public function addMediaButton( $page = null, $target = null ) {
		echo '<a href="#TB_inline?width=640&height=600&inlineId=aletheme-slider-editor-shortcode-wrap" 
		         class="thickbox" 
		         title="' . __( 'Insert Slider', 'aletheme' ) . '" 
		         data-page="' . $page . '" 
		         data-target="' . $target . '"><img 
		         src="' . ALETHEME_URL . '/assets/images/aletheme_icon_16.png" 
		         alt="Insert Slider" />
		      </a>';
	}
					...
					<select id="aletheme-slider-editor-shortcode-select">
						...
						<?php foreach ($this->getList() as $slider) : ?>
							<?php if ($slider->post_status == 'publish') : ?>
								<option value="<?php echo $slider->post_name ?>">
				...
				<div id="aletheme-slider-editor-shortcode-settings">
					<div id="aletheme-slider-info">
						...
						<dl class="shortcode" data-slug="<?php echo $slider->post_name ?>">
							...
							<dd>
                                <input type="button"
                                       id="aletheme-slider-editor-shortcode-insert"
                                       value="Insert Slider"
                                       class="button-primary aligncenter" />
                            </dd>
						</dl>
						<div class="clear"></div>
...
function ale_sliders_get_slider($slug) {
	...
	$width = false;
	$height = false;
	if (isset($sliders_config[$slider->post_name]) &&
        isset($sliders_config[$slider->post_name]['width']) &&
        isset($sliders_config[$slider->post_name]['height'])) {
