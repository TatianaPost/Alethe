	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$output .= '<div class="no_image">
                                <span class="file_link"><a href="' . $value . '" target="_blank" rel="external">'
                                    .$title.'</a>
                                </span>'
                                . $remove .
                           '</div>';
...
if ( ! function_exists( 'optionsframework_mlu_get_silentpost' ) ) {
	...
	function optionsframework_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type'      => 'optionsframework',
                            'post_name'      => 'of-' . $_token,
                            'post_status'    => 'draft',
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'optionsframework' ),
                                        __( 'Previously Uploaded', 'optionsframework' ),
                                        $tabs['gallery'] );
		return $tabs;
	}
}