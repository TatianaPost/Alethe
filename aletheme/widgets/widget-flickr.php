...
class Aletheme_Flickr_Widget extends WP_Widget 
{
	...
	{
		/* Widget settings. */
		$widget_ops  = array( 'classname'   => 'aletheme-flickr',
                              'description' => 'Display a Flickr Photostream' );
		...
		?>
			...
			<p>
				...
				<select name="<?php echo $this->get_field_name( 'num_items' ); ?>" id="<?php echo $this->get_field_id( 'num_items' ); ?>">
					...
					<?php for ($i=1; $i<=20; $i++) { ?>
						<option <?php if ($instance['num_items'] == $i) { echo 'selected'; } ?> value="<?php echo $i; ?>">
                            <?php echo $i; ?>
                        </option>
					<?php } ?>
				</select>
			...
			<p class="id_parent">
				...
				<input name="<?php echo $this->get_field_name( 'id' ); ?>"
                       type="text" id="<?php echo $this->get_field_id( 'id' ); ?>"
                       value="<?php echo $instance['id']; ?>" size="20" />
			</p>
			<p class="set_parent">
				<label for="<?php echo $this->get_field_id( 'set' ); ?>">Set ID</label>
				<input name="<?php echo $this->get_field_name( 'set' ); ?>"
                       type="text" id="<?php echo $this->get_field_id( 'set' ); ?>"
                       value="<?php echo $instance['set']; ?>"
                       size="40" />
			...
			<p class="tags_parent">
				...
				<input class="widefat"
                       ...
                       type="text" id="<?php echo $this->get_field_id( 'tags' ); ?>"
			...
			<p>
				...
				<input name="<?php echo $this->get_field_name( 'do_cache' ); ?>"
                       type="checkbox"
                       id="<?php echo $this->get_field_id( 'do_cache' ); ?>"
                    <?php echo $instance['do_cache']==1?'checked="checked"':''; ?>
                       value="1" />
			</p>
		<?php
	}
