...
class Aletheme_WidgetImageField 
{
	...
	function __construct($widget = null, $image_id = 0) {
		...
		if ($uri && in_array($file, array('widgets.php')) && is_admin()) {
			...
			wp_enqueue_script('widgetimagefield',
							   ALETHEME_URL . '/assets/js/widgets.js',
	...
	function get_widget_field($field_name = null) {
		...
		if ($this->widget && ( isset($this->widget->image_field) || $field_name )) {
			...
			$field .= "<input type='hidden' 
							  style='display:none;' 
							  id='" . $this->widget->get_field_id($this->widget->image_field) . "' 
							  name='" . $this->widget->get_field_name($this->widget->image_field) . "' 
							  value='" . $this->image_id . "' />";
			...
			$field .= "<a class='button iti-image-widget-trigger' 
						  href='media-upload.php?TB_iframe=1&amp;width=640&amp;height=1500' 
						  title='Choose Image'>";
			$field .= "Choose Image";
			$field .= "</a></div>";
		}
