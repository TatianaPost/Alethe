...
class Ale_Meta_Box {
	...
	function add_for_id( $display, $meta_box ) {
		...
		$meta_box['show_on']['value'] = !is_array( $meta_box['show_on']['value'] ) ?
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'text_small':
					echo '<input class="ale_text_small" type="text" name="',
                         $field['id'],
                         '" id="',
                         $field['id'],
                         '" value="',
                         '' !== $meta ? $meta : $field['std'], '" />
                        <span class="ale_metabox_description">', $field['desc'], '</span>';
					break;

				case 'text_medium':
					echo '<input class="ale_text_medium" type="text" name="',
                         $field['id'],
                         '" id="',
                         $field['id'],
                         '" value="',
				...
				case 'text_date':
					echo '<input class="ale_text_small ale_datepicker" type="text" name="',
                         $field['id'],
                         '" id="',
                         $field['id'],
                         '" value="',
				...
				case 'text_date_timestamp':
					echo '<input class="ale_text_small ale_datepicker" type="text" name="',
                         ...
                         $field['id'],
                         '" value="',
                         '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />
                        <span class="ale_metabox_description">', $field['desc'], '</span>';
					break;
				...
				case 'text_datetime_timestamp':
					echo '<input class="ale_text_small ale_datepicker" type="text" name="',
                         ...
                         '_date" value="',
                         '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />';
					echo '<input class="ale_timepicker text_time" type="text" name="',
                         $field['id'],
                         '[time]" id="',
                         $field['id'],
                         '_time" value="',
                         '' !== $meta ? date( 'h:i A', $meta ) : $field['std'], '" />
                        <span class="ale_metabox_description" >', $field['desc'], '</span>';
					break;
				...
				case 'text_money':
					echo '$ <input class="ale_text_money" type="text" name="',
                         $field['id'],
                         '" id="',
                         $field['id'],
                         '" value="',
				...
				case 'colorpicker':
					echo '<div id="' . esc_attr( $field['id'] . '_picker' ) . '" class="colorSelector">
					        <div style="' . esc_attr( 'background-color:' . ('' !== $meta ? $meta : $field['std']) ) . '">
					        </div>
					     </div>';
					echo '<input class="ale_colorpicker ale_text_small" name="' .
                                 $field['id'] .
                                 '" id="' .
                                 esc_attr($field['id']) .
                                 '" type="text" value="',
				...
				case 'textarea_code':
					echo '<textarea name="',
                                    $field['id'],
                                    '" id="', $field['id'],
                                    '" cols="60" rows="10" class="ale_textarea_code">',
                                    '' !== $meta ? $meta : $field['std'],
                        '</textarea>',
                        '<p class="ale_metabox_description">', $field['desc'], '</p>';
					break;
				...
				case 'select':
					...
					foreach ($field['options'] as $option) {
						echo '<option value="', $option['value'], '"',
                                      $meta == $option['value'] ? ' selected="selected"' : '', '>',
                                        $option['name'],
                            '</option>';
					}
					echo '</select>';
				...
				case 'custom_post_select':
				    ...
				    foreach ( $items as $item ) {
                       echo '<option value="', $item->ID, '"', $meta == $item->ID ? ' selected="selected"' : '', '>',
                                $item->post_title,
                            '</option>';
				    }
				    echo '</select>';
				...
				case 'radio_inline':
					...
					$i = 1;
					foreach ($field['options'] as $option) {
						echo '<div class="ale_radio_inline_option">
                                <input type="radio" name="', $field['id'], '" id="', $field['id'], $i, '" value="', $option['value'], '"', $meta == $option['value'] ? ' checked="checked"' : '', ' />
                                <label for="', $field['id'], $i, '">', $option['name'], '</label>
                              </div>';
						$i++;
					}
					echo '</div>';
				...
				case 'radio':
					...
					$i = 1;
					foreach ($field['options'] as $option) {
						echo '<li><input type="radio" name="', $field['id'], '" id="', $field['id'], $i,'" value="', $option['value'], '"', $meta == $option['value'] ? ' checked="checked"' : '', ' />
						        <label for="', $field['id'], $i, '">', $option['name'].'</label>
                              </li>';
						$i++;
					}
				...
				case 'multicheck':
					...
					foreach ( $field['options'] as $value => $name ) {
						...
						echo '<li><input type="checkbox" name="', $field['id'], '[]" id="', $field['id'], $i, '" value="', $value, '"', in_array( $value, $meta ) ? ' checked="checked"' : '', ' />
						        <label for="', $field['id'], $i, '">', $name, '</label>
                              </li>';
						$i++;
					}
					echo '</ul>';
					...
					echo '<div id="', $field['id'], '_status" class="ale_upload_status">';	
						if ( $meta != '' ) { 
							...
							if ( $check_image ) {
								...
								echo 'File: <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; 
                                            (<a href="', $meta, '" target="_blank" rel="external">Download</a> / 
                                             <a href="#" class="ale_remove_file_button" rel="', $field['id'], '">Remove</a>)';
							}	
						}
					echo '</div>'; 
				break;
