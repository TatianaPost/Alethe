...
<?php elseif (ale_get_option('comments_style') == 'wp') : ?>
    ...
    <div class="blog-comments">
        ...
        <div class="comments-header">
            ...
            <p class="right">
                <a id="hide-show-button"><?php _e('Hide / Show Comments','aletheme'); ?></a>
            </p>
        </div>
            ...
            <?php if (comments_open()) : ?>
                ...
                <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" id="comment-form" method="post" class="comment-form">
                    ...
                    <?php if (is_user_logged_in()) : ?>
                        <p>
                            <?php printf(__('Logged in as <a href="%s/wp-admin/profile.php">%s</a>.', 'aletheme'), get_option('siteurl'), $user_identity); ?> <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php __('Log out of this account', 'aletheme'); ?>"><?php _e('Log out &raquo;', 'aletheme'); ?></a></p>
                    <?php else : ?>
                    <div class="personal">
                        <input type="text"
                               class="name"
                               placeholder="<?php _e('Your Name', 'aletheme'); ?> *"
                               name="author" id="author" value="<?php echo esc_attr($comment_author); ?>"
                               tabindex="1" <?php if ($req) echo "aria-required='true'"; ?>
                               required="required">
                        <input type="email"
                               placeholder="<?php _e('Your Email', 'aletheme'); ?> *"
                               class="email"
                               name="email"
                               id="email"
                               ...
                               email="true">
                        <input type="url"
                               placeholder="<?php _e('Your Site', 'aletheme'); ?>"
                               class="site"
                               name="url"
                               id="url"
